...
export async function createLoan(input: {
  ...
  const dcrUsd = Number.isFinite(input.dcrUsd) && input.dcrUsd ? input.dcrUsd : 12.13;
  const collateralUsd = Number((input.collateralDcr * dcrUsd).toFixed(2));
  const ltv = Math.round((input.borrowAmount / collateralUsd) * 10000);
  const platformFeeAmount = Number((input.borrowAmount * 0.01).toFixed(2));
  const loan: Loan = {
    id,
    ref,
...
export async function updateLoan(loan: Loan, event: Omit<LoanEvent, "id">): Promise<{ loan: Loan; event: LoanEvent }> {
  ...
  run(
    db,
    `UPDATE loans SET
      status = ?, collateralUsd = ?, currentLtvBps = ?, platformFeeBps = ?, platformFeeAmount = ?, depositTxid = ?, payoutTxid = ?,
...
function migrate(db: Database): void {
  db.run(`
    CREATE TABLE IF NOT EXISTS loans (
      ...
      platformFeeAmount REAL NOT NULL DEFAULT 0,
  ...
  `);
  addColumnIfMissing(db, "loans", "platformFeeBps", "INTEGER NOT NULL DEFAULT 100");
  addColumnIfMissing(db, "loans", "platformFeeAmount", "REAL NOT NULL DEFAULT 0");
}
...
function addColumnIfMissing(db: Database, table: string, column: string, definition: string): void {
  const columns = rows<{ name: string }>(db, `PRAGMA table_info(${table})`);
