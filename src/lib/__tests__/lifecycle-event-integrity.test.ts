...
describe("lifecycle event integrity gate", () => {
  ...
  it("no-ops duplicate event ids without replacing the accepted event", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-DUP-ID");
    const input = {
      lookupCode: record.lookupCode,
    ...
    const first = await submitHeadlessLifecycleEvent(input, { lifecycleStore, eventStore });
    const second = await submitHeadlessLifecycleEvent(input, { lifecycleStore, eventStore });
  ...
  it("no-ops duplicate watcher event ids for the same lifecycle event kind", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-WATCHER");
  ...
  it("no-ops duplicate oracle health result ids for the same event kind", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-HEALTH");
  ...
  it("no-ops duplicate arbiter decision ids", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-ARB-DUP");
  ...
  it("blocks stale watcher events from overwriting newer confirmed state", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-STALE-WATCHER");
  ...
  it("blocks stale oracle health results that would make the loan safer", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-STALE-ORACLE");
  ...
  it("blocks repayment mismatches from completing repayment", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-REPAY-MISMATCH");
  ...
  it("blocks older partial repayment from rolling back full repayment", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-REPAY-ROLLBACK");
  ...
  it("blocks older arbiter request from reopening resolved review", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-ARB-ORDER");
  ...
  it("blocks liquidation review events that imply execution readiness", async () => {
    const { lifecycleStore, eventStore, record } = setup("DCL-INTEGRITY-LIQ-UNSAFE");
