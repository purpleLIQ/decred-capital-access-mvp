...
const eventSubmitSchema = z.object({
  ...
  payload: z.object({
    ...
    health: liquidationHealthStatusSchema.optional(),
    ...
    healthResultId: z.string().trim().max(160).optional(),
    policyVersion: z.string().trim().max(160).optional(),
    oracleObservationIds: z.array(z.string().trim().max(160)).optional(),
    selectedDcrUsdPrice: z.coerce.number().nonnegative().optional(),
    selectedBorrowAssetUsdPrice: z.coerce.number().nonnegative().optional(),
    oracleSourceCount: z.coerce.number().int().nonnegative().optional(),
    oracleFreshnessStatus: z.string().trim().max(80).optional(),
    oracleDeviationStatus: z.string().trim().max(80).optional(),
    oracleQuorumStatus: z.string().trim().max(80).optional(),
    oracleUsable: z.boolean().optional(),
    oracleBlockerReason: z.string().trim().max(1000).optional(),
    ltvBps: z.coerce.number().nonnegative().optional(),
    collateralizationBps: z.coerce.number().nonnegative().optional(),
    collateralValueUsd: z.coerce.number().nonnegative().optional(),
    debtValueUsd: z.coerce.number().nonnegative().optional(),
    warningWindowStatus: warningWindowStatusSchema.optional(),
    warningDeadline: z.string().datetime().optional(),
    topUpPlaceholderAmountDcr: z.coerce.number().nonnegative().optional(),
    borrowerSafeSummary: z.string().trim().max(1000).optional(),
    operatorInternalSummary: z.string().trim().max(1000).optional(),
    nextBorrowerAction: z.string().trim().max(1000).optional(),
    nextOperatorArbiterAction: z.string().trim().max(1000).optional(),
    ...
    integrityStatus: lifecycleEventIntegrityStatusSchema.optional(),
